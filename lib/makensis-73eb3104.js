...
var eventEmitter = new require$$2.EventEmitter();
...
function mapArguments(args, options) {
    ...
    var pathToWine = options.pathToWine
        ...
        : 'wine';
    var cmd;
    if (require$$0$2.platform() !== 'win32' && options.wine === true) {
        cmd = pathToWine;
    ...
    if (options.priority) {
        ...
        if (require$$0$2.platform() === 'win32' && isNumeric(priority) && inRange(priority, 0, 5)) {
...
function splitLines(input, opts) {
    if (opts === void 0) { opts = {}; }
    var lineBreak = (require$$0$2.platform() === 'win32' || opts.wine === true) ? '\r\n' : '\n';
...
function spawnMakensis(cmd, args, compilerOptions, spawnOptions) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        var child = require$$1.spawn(cmd, args, spawnOptions);
...
function spawnMakensisSync(cmd, args, compilerOptions, spawnOptions) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    var child = require$$1.spawnSync(cmd, args, spawnOptions);
...
function getMagicEnvVars(envFile) {
    expand_1(main$1.exports.config({
...
function findEnvFile(dotenvPath) {
    if (typeof dotenvPath === 'string' && (dotenvPath === null || dotenvPath === void 0 ? void 0 : dotenvPath.length) && require$$0.existsSync(dotenvPath) && require$$0.lstatSync(dotenvPath).isFile()) {
        return dotenvPath;
    }
    ...
    if (cwd) {
        switch (true) {
            case (require$$0.existsSync(require$$0$1.join(cwd, ".env.[".concat(process.env.NODE_ENV, "].local")))):
                dotenvFile = require$$0$1.join(cwd, ".env.[".concat(process.env.NODE_ENV, "].local"));
                break;
            case (require$$0.existsSync(require$$0$1.join(cwd, '.env.local'))):
                dotenvFile = require$$0$1.join(cwd, '.env.local');
                break;
            case (process.env.NODE_ENV && require$$0.existsSync(require$$0$1.join(cwd, ".env.[".concat(process.env.NODE_ENV, "]")))):
                dotenvFile = require$$0$1.join(cwd, ".env.[".concat(process.env.NODE_ENV, "]"));
                break;
            case (require$$0.existsSync(require$$0$1.join(cwd, '.env'))):
                dotenvFile = require$$0$1.join(cwd, '.env');
                break;
        }
    }
    return dotenvFile;
...
//# sourceMappingURL=makensis-73eb3104.js.map
