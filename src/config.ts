...
export default {
	schema: {
		...
		showBuildNotifications: {
			...
			description:
				"Displays color-coded notifications that close automatically after 5 seconds",
			type: "boolean",
			default: true,
			order: 2,
		},
		...
		alwaysShowOutput: {
			...
			description:
				"Displays compiler output in console panel. When deactivated, it will only show on errors",
			type: "boolean",
			default: true,
			order: 4,
		},
		...
		compilerOutput: {
			...
			description:
				"Specify whether `makensis` outputs its version or compiler flags to notifications the console",
			type: "string",
			default: "console",
			enum: [
				{
		...
		buildFileSyntax: {
			...
			description:
				"Specify the default syntax for your build file (requires [build](https://atom.io/packages/build))",
			type: "string",
			default: "json",
			enum: [
				{
	...
	migrate(oldKey: string, newKey: string): void {
		if (
			!atom.config.get(`${name}.${oldKey}`) ||
			atom.config.get(`${name}.${newKey}`)
		) {
			return;
		}
		...
		try {
			...
			atom.notifications.addWarning(
				`Failed to migrate configuration, see console for details`,
			);
